...
function checkpitch(){
  ...
  // 發送 request 到 後端
  fetch(`/check-pitcher?name=${encodeURIComponent(pitcherName)}`)
      .then(response => response.json()) // 將回傳值轉為 .json
      .then(result => {
        if(result.exists){
            // 把所有投手列在前端
            displayPitcherOptions(result.pitchers);
        }
        else{
          alert('投手不存在, 請重新輸入！');
        }
...
function displayPitcherOptions(pitchers) {
  console.log("display");
  ...
  pitchers.forEach(pitcher => {
    const buttonHTML = `
        <button 
            id="pitcher-btn-${pitcher.player_unique_id}" 
        ...
        >
            ${pitcher.name} (${pitcher.team}) ${pitcher.height}cm ${pitcher.weight}kg <br>
            背號: ${pitcher.number} | 國籍: ${pitcher.nationality} | 位置: ${pitcher.position} <br>
        </button>
    `;
  ...
  // 為每個按鈕添加事件監聽器
  pitchers.forEach(pitcher => {
    const button = document.getElementById(`pitcher-btn-${pitcher.player_unique_id}`);
    button.onclick = () => selectPitcher(pitcher);
  });
...
function selectPitcher(pitcher){
  alert(`您選擇了 ${pitcher.name} (${pitcher.team})`);

  selectedPitcherId = pitcher.player_unique_id;
  // 顯示選擇的投手資訊
  const selectedPitcherDiv = document.getElementById('selected-pitcher');
  selectedPitcherDiv.innerHTML = `您選擇了 <strong>${pitcher.name}</strong> (${pitcher.team}) ${pitcher.height}cm ${pitcher.weight}kg 背號:${pitcher.number}`;
...
function checkbatter() {
  ...
  // 發送 request 到後端
  fetch(`/check-batter?name=${encodeURIComponent(batterName)}`)
      .then(response => response.json()) // 將回傳值轉為 .json
      .then(result => {
        console.log(result); // 確認後端返回的數據結構
          if (result.exists) {
              // 把所有打者顯示在前端
              displayBatterOptions(result.batters);
          } else {
              alert('打者不存在，請重新輸入！');
          }
      })
...
function displayBatterOptions(batters) {
  ...
  batters.forEach(batter => {
      const buttonHTML = `
          <button 
              id="batter-btn-${batter.player_unique_id}" 
              style="display: block; width: 100%; margin-bottom: 10px; padding: 10px; font-size: 16px; background-color: #007bff; color: #fff; border: none; border-radius: 5px; cursor: pointer;"
          >
              ${batter.name} (${batter.team}) ${batter.height}cm ${batter.weight}kg <br>
              背號: ${batter.number} | 國籍: ${batter.nationality} | 位置: ${batter.position} <br>
          </button>
      `;
      optionDiv.innerHTML += buttonHTML;
  });
  ...
  // 為每個按鈕添加事件監聽器
  batters.forEach(batter => {
      const button = document.getElementById(`batter-btn-${batter.player_unique_id}`);
      button.onclick = () => selectBatter(batter);
  });
...
function selectBatter(batter) {
  selectedBatterId = batter.player_unique_id;

  alert(`您選擇了 ${batter.name} (${batter.team})`);

  // 顯示選擇的打者資訊
  const selectedBatterDiv = document.getElementById('selected-batter');
  selectedBatterDiv.innerHTML = `您選擇了 <strong>${batter.name}</strong> (${batter.team}) ${batter.height}cm ${batter.weight}kg 背號:${batter.number}`;
