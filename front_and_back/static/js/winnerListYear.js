const chooseYearButtonList = document.querySelector('.chooseYearButtonList');
...
chooseLeft.addEventListener('click', () => {
    ...
    {
        posX += translateVal;
        chooseYearButtonList.style.transform = `translateX(${posX}px)`;
    }
});
...
searchYearButton.forEach(button => {
    button.addEventListener('click', async (event) => {
        ...
        try {
            const winnerResponse = await fetch(`/winnerlist/searchyear?years=${yearVal}`);
            ...
            const fetchPlayer = async (playerId) => {
                const playerResponse = await fetch(`/players/searchplayer?player_unique_id=${playerId}`);
